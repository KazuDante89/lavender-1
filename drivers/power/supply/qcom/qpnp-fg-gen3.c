...
static int fg_get_batt_profile(struct fg_chip *chip)
{
	...
	if (hwc_check_global){
		pr_debug("sunxing get global set fastchg  2.3A");
...
static irqreturn_t fg_delta_msoc_irq_handler(int irq, void *data)
{
	...
	if (!rc)
		pr_debug("lct battery SOC:%d voltage:%duV current:%duA temp:%d id:%dK charge_status:%d charge_type:%d health:%d input_present:%d \n",
...
static int fg_gen3_probe(struct platform_device *pdev)
{
	...
	if (!rc) {
		pr_debug("battery SOC:%d voltage: %duV temp: %d id: %dKOhms\n",
